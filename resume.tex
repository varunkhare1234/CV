...
\item \textbf{Visiting Research Scholar} (National University Singapore)\\
 \emph{(Guide: Prof. Tat Seng Chua, May'18 - July'18)} \\[-0.6cm]
	...
	\item Proposed a novel end-to-end architecture consisting of two modules for robust pose prediction and classification based on \textbf{2.5 D sketches} proposed in \textbf{Marr's theory of vision} 
	\item One sub module learns to \textbf{reconstruct 3D model} in its canonical viewpoint via \textbf{multi-task learning DNN}s.
	\item Another NN sub module uses \textbf{Faster R-CNN} style anchor boxes to predict the \textbf{6 DoF} poses in \textbf{continuous domain} which \textbf{outperforms} PnP based solvers due to robust multi-class classification of 3D objects
